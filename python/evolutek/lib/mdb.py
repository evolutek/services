...
class Mdb:
    def __init__(self):
        ...
        try:
            ...
            print(
                "[MDB] ERROR couldn't initialise i2c bus between Teensy and RaspberryPI"
            )
            raise (e)
    ...
    def send(self, msg):
        try:
            self.i2c.writeto(TEENSY, msg)
        except:
            print("[MDB] ERROR while sending message: " + str(msg))
    ...
    def get_zones(self):
        ...
        for i in range(len(buf)):
            if buf[i] not in [0, 1]:
                err = True
        if err:
            print("[MDB] ERROR: get_zones received erroneous data! " + str(buf))

        return {"front": buf[0] == 1, "back": buf[1] == 1, "is_robot": buf[2] == 1}
