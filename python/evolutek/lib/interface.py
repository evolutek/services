...
class Interface:
    def __init__(self, title, nb_lines=1):
        ...
        img = img.resize(
            (int(3000 * self.interface_ratio), int(2000 * self.interface_ratio)),
            Image.ANTIALIAS,
        )
    ...
    def init_robot(self, robot):
        self.robots[robot] = {
            "telemetry": None,
            "size": float(self.cs.config.get(section=robot, option="robot_size_y")),
            "color": "orange",
        }
        ...
        setattr(
            self,
            "%s_watchdog" % robot,
            Watchdog(self.timeout_robot * 2, self.reset_robot, [robot]),
        )
