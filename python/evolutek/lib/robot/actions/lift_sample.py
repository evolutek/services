...
def lift_sample(self):
    status = RobotStatus.get_status(self.goto_avoid(340, 1975, async_task=False, timeout=10))
    ...
    status = RobotStatus.get_status(self.goto_avoid(340, 1620, async_task=False, timeout=10))
    if RobotStatus.get_status(status) != RobotStatus.Reached:
        return RobotStatus.return_status(RobotStatus.get_status(status))
