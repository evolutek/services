...
@if_enabled
@async_task
def goto_avoid_extend(self, x, y, avoid=True, timeout=None, skip=False, mirror=True, dec=None, acc=None, speed=None):
...
def global_goto_avoid(self, x, y, theta, avoid=True, timeout=None, skip=False, mirror=True,
    ...
    while status != RobotStatus.Reached:
        ...
        data = self.goto_global(x, y, theta, avoid=avoid, mirror=False, async_task=False)
        status = RobotStatus.get_status(data)
