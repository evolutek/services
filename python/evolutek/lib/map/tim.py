...
class Tim:
        ...
        while True:
            while not self.connected:
                try:
                    ...
                    print(
                        "[TIM] Failed to connect to the TIM %s: %s" % (self.ip, str(e))
                    )
                    self.connected = False
                    sleep(1)
    ...
    def change_pos(self, mirror=False):
        self.pos = Point(
            self.default_pos.x,
            3000 - self.default_pos.y if mirror else self.default_pos.y,
        )
        self.angle = self.default_angle * -1 if mirror else self.default_angle
        print(
            "[TIM] Changing tim %s pos: %s, angle: %d" % (self.ip, self.pos, self.angle)
        )
    ...
    def scan(self):
        ...
        raw_points = self.convert_to_card(
            list(map(parse_num, data[26 : 26 + length])), angular_step
        )
        ...
        for shape in shapes:
            clouds.append(
                RobotCloud(shape, ip=self.ip, center=self.compute_center(shape))
            )
