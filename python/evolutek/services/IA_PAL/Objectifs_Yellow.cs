class Objectifs_Yellow
{
	...
	{
		Objectif plot_haut_gauche = new Objectif("plot_haut_gauche", 100, 100, (3*pi)/2, true, false);
		objectifs.add(plot_haut_gauche);
		Objectif plot_haut_droite = new Objectif("plot_haut_droite", 150, 450, pi/2, true, false);
		objectifs.add(plot_haut_droite);
		Objectif plot_bas_gauche = new Objectif("plot_bas_gauche", 1800, 100, (3*pi)/2, false, false);
		objectifs.add(plot_bas_gauche);
		Objectif plot_bas_droite = new Objectif("plot_bas_droite", 1700, 1350, pi/3, true, false);
		objectifs.add(plot_bas_doite);
		Objectif clap_gauche = new Objectif("clap_gauche", 1900, 100, pi/2, false, false);
		objectifs.add(clap_gauche);
		Objectif clap_droit = new Objectif("clap_droit", 1900, 2900, (3*pi)/2, true, false);
		objectifs.add(clap_droit);
		Objectif recup_balle = new objectif("recup_balle", 1000, 50, (3*pi)/2, true, false);
		...
		Objectif pose_pile_droite = new Objectif("pose_pile_droite", 1900, 1350, 0, false, false);
		objectifs.add(pose_pile_droite);
		Objectif ramasse_gobelet_bas_gauche = new Objectif("ramasse_gobelet_bas_gauche", 1900, 300, pi/2, false, false);
		objectifs.add(ramasse_gobelet_bas_gauche);
	...
	public void do_objectif(Objectif objectif)
	{
		switch(objectif.name)
		{
			...
			case "plot_bas_gauche":
				...
				objectif.finished = true;
				objectifs.find(clap_gauche).possible = true;
				break;
			...
			case "ramasse_gobelet_bas_gauche":
				goto-xy(objectif.x, objectif.y);
				goto-teta(objectif.teta);
				ramasse_gobelet();
				objectif.finished = true;
				objectifs.find(pose_gobelet_haut).possible = true;
				objectifs.find(plot_bas_gauche).possible = true;
				break;
