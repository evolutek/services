...
from evolutek.lib.map.map import ObstacleType, parse_obstacle_file
...
from evolutek.lib.map.utils import (
    convert_path_to_dict,
    convert_path_to_point,
    merge_polygons,
)
...
class Map(Service):
    def __init__(self):
        ...
        fixed_obstacles, self.color_obstacles = parse_obstacle_file(
            "/etc/conf.d/obstacles.json"
        )
        ...
        self.delta_dist = (
            float(self.cs.config.get(section="tim", option="delta_dist")) * 2
        )
        self.pal_telem = None
        self.pmi_telem = None
        self.clouds = []
    ...
    def match_color(self, color):
        ...
        for tim in self.tim_config:
            if tim["ip"] in self.tims and not self.tims[tim["ip"]] is None:
                ...
                self.tims[tim["ip"]] = Tim(
                    tim, self.tim_computation_config, self.color != self.color1
                )
    ...
    def get_path(self, origin, dest, robot):
        ...
        with self.lock:
            ...
            if robot not in ["pal", "pmi"]:
                print(
                    "[MAP] WARNING: Unknown robot "
                    + robot
                    + ". Possible values are pal and pmi"
                )
            else:
                ...
                if getattr(self, "%s_telem" % other_robot) is None:
                    print(
                        "[MAP] WARNING: %s telemetry not known, calculating a path ignoring it"
                        % other_robot
                    )
                else:
    ...
    def is_path_valid(self, path, robot):
        with self.lock:
            ...
            if robot not in ["pal", "pmi"]:
                ...
                if getattr(self, "%s_telem" % other_robot) is None:
                    print(
                        "[MAP] WARNING: %s telemetry not known, checking valididy ignoring it"
                        % other_robot
                    )
                else:
                    self.map.add_octogon_obstacle(
    ...
    def loop_scan(self):
        while True:
            ...
            for ip in self.tims:
                ...
                for new in scan:
                    ...
                    if not merged:
                        ...
                        if (
                            self.pal_telem
                            and new.merged_pos.dist(self.pal_telem) < self.delta_dist
                        ):
                            new.tag = "PAL"
                            new.add_telemetry(self.pal_telem)
                        elif (
                            self.pmi_telem
                            and new.merged_pos.dist(self.pmi_telem) < self.delta_dist
                        ):
                            new.tag = "PMI"
            ...
            with self.lock:
                ...
                for robot in robots:
                    tag = "robot%d" % i
                    if self.map.add_octogon_obstacle(
                        robot, self.robot_size, tag=tag, type=ObstacleType.robot
                    ):
                        d = robot.to_dict()
                        d["tag"] = tag
                        i += 1
