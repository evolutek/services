...
from evolutek.lib.robot import DELTA_POS, Robot, Status
...
class Ai:
    def __init__(self):
        ...
        self.fsm = Fsm(States)
        self.fsm.add_state(
            States.Setup, self.setup, prevs=[States.Waiting, States.Ending]
        )
        self.fsm.add_state(States.Waiting, self.waiting, prevs=[States.Setup])
        self.fsm.add_state(
            States.Selecting, self.selecting, prevs=[States.Waiting, States.Making]
        )
        self.fsm.add_state(
            States.Making, self.making, prevs=[States.Selecting, States.Making]
        )
        self.fsm.add_state(
            States.Ending,
            self.ending,
            prevs=[States.Setup, States.Waiting, States.Selecting, States.Making],
        )
        self.fsm.add_error_state(self.error)

        Gpio(17, "tirette", False, edge=Edge.FALLING).auto_refresh(
            callback=self.handle_tirette
        )
    ...
    def setup(self):
        ...
        if self.goals.critical_goal is not None:
            self.critical_timer = Timer(
                self.goals.timeout_critical_goal, self.critical_timeout_handler
            )
    ...
    def selecting(self):
        ...
        if self.critical.is_set():
            ...
            if (
                not self.goals.critical_goal is None
                and self.current_goal.name == self.goals.critical_goal
            ):
    ...
    def making(self):
        ...
        status = None
        pos = self.robot.mirror_pos(
            x=self.current_goal.position.x, y=self.current_goal.position.y
        )
        pos = {"x": pos[0], "y": pos[1]}
        ...
        if Point.dist_dict(pos, current_pos) < DELTA_POS:
            ...
            if self.use_pathfinding:
                ...
                status = self.robot.goto_with_path(
                ...
                while status != Status.reached:
                    ...
                    if (
                        self.current_goal.timeout is not None
                        and t > self.current_goal.timeout
                    ) or self.current_goal.secondary_goal is not None:

                        self.current_goal = self.goals.get_secondary(
                            self.current_goal.secondary_goal
                        )
        ...
        if not self.current_goal.theta is None:
            ...
            print(
                "[AI] Going to %s"
                % self.robot.mirror_pos(theta=self.current_goal.theta)[2]
            )
        ...
        for action in self.current_goal.actions:
            ...
            if action.score > 0:
                self.robot.client.publish(
                    "score", data=json.dumps({"value": action.score}).encode()
                )
        ...
        if self.current_goal.score > 0:
            self.robot.client.publish(
                "score", data=json.dumps({"value": goal_score}).encode()
            )
