...
make_setting("TRAJMAN_BAUDRATE", 38400, "trajman", "baudrate", "TRAJMAN_BAUDRATE", int)
from cellaserv.settings import TRAJMAN_BAUDRATE, TRAJMAN_PORT
from evolutek.lib.gpio import Edge as GpioEdge
...
def if_enabled(method):
    ...
    def wrapped(self, *args, **kwargs):
        if self.disabled:
            self.log(what="disabled", msg="Usage of {} is disabled".format(method))
            return
...
class TrajMan(Service):
    ...
    telemetry_refresh = ConfigVariable(
        section=ROBOT, option="telemetry_refresh", coerc=float
    )
    ...
    def error_mdb(self, enable=True):
        ...
        if enable:
            ...
            print("[AVOID] Disabling error mode on MDB")
    ...
    @Service.action
    def status(self):
        return {"disabled": self.disabled, "moving": self.is_moving()}
    ...
    def move_trsl(self, dest, acc, dec, maxspeed, sens):
        ...
        tab += pack(
            "ffffb", float(dest), float(acc), float(dec), float(maxspeed), int(sens)
        )
        self.command(bytes(tab))
    ...
    @Service.action
    def set_mdb_config(
        self, mode=None, yellow=None, near=None, far=None, brightness=None
    ):
        if mode is not None:
        ...
        while True:
            ...
            if len(tab) > 1:
                if tab[1] == Commands.ACKNOWLEDGE.value:
                    ...
                    a, b, tracc, trdec, trmax, rtacc, rtdec, rtmax = unpack(
                        "=bbffffff", bytes(tab)
                    )
                    ...
                    self.log_serial(
                        "Rotation:    Acc: {0:.3f}\tDec: {1:.3f}\tMax: {2:.3f} (rad/(s*s) rad/(s*s) rad/s)".format(
                            rtacc, rtdec, rtmax
                        )
                    )

                elif tab[1] == Commands.GET_WHEELS.value:
                    a, b, spacing, left_diameter, right_diameter = unpack(
                        "=bbfff", bytes(tab)
                    )
                    ...
                    self.log_serial(
                        "delta_rot_max : ", rotation, "delta_trsl_max", translation
                    )
                    ...
                    a, b, recal_xpos, recal_ypos, recal_theta = unpack(
                        "=bbfff", bytes(tab)
                    )
                    ...
                    (
                        ...
                        rti,
                        rtd,
                    ) = unpack("=bbfffffffffffffff", bytes(tab))
                    try:
                        if self.debug_file and not self.debug_file.closed:
                            self.debug_file.write(str(time) + " ")
                            self.debug_file.write(
                                str(xpos) + " " + str(ypos) + " " + str(theta) + " "
                            )
                            self.debug_file.write(
                                str(wpx) + " " + str(wpy) + " " + str(wpth) + " "
                            )
                            self.debug_file.write(
                                str(trspeed) + " " + str(rotspeed) + " "
                            )
                            self.debug_file.write(
                                str(trp) + " " + str(tri) + " " + str(trd) + " "
                            )
                    ...
                    counter, commandid, xpos, ypos, theta, speed = unpack(
                        "=bbffff", bytes(tab)
                    )
                    self.telemetry = {
                        "x": xpos,
