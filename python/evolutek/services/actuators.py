...
from evolutek.lib.gpio import Edge, Gpio, Pwm
...
class Actuators(Service):
    ...
    def pumps_drop(self, pumps):
        ...
        try:
            self.queue.launch_multiple_actions(
                _pumps, [[] for i in range((len(pumps)))]
            )
    ...
    def drop_starting_with_sort(self):
        ...
        if self.queue.stop.is_set():
            return (
                Status.unreached.value
                if self.should_stop(status)
                else Status.reached.value
            )
    ...
    def drop_center_zone(self):
        ...
        if (pattern == 3) ^ side:
            ...
            self.right_cup_holder_drop()
        if self.queue.stop.is_set():
            self.left_cup_holder_close() if (
                pattern == 3
            ) ^ side else self.right_cup_holder_close()
            return Status.unreached.value
        sleep(0.5)
