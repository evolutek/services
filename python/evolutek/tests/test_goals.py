...
class Test_Goals:
    def __init__(self):
        ...
        print(
            "[TEST] Testing of critical goal success: %s"
            % str(self.eval_critical_goal())
        )
    ...
    def eval_b(self):
        ...
        if action.avoid_strategy != AvoidStrategy.Wait:
            print(
                "[TEST] sleep action has bad avoid strategy: %s"
                % action.avoid_strategy.name
            )
            passed = False
        ...
        if action.avoid_strategy != AvoidStrategy.Timeout:
            print(
                "[TEST] test action has bad avoid strategy: %s"
                % action.avoid_strategy.name
            )
            passed = False
    ...
    def eval_strategies(self):
        ...
        if len(self.goals.strategies) != 2:
            print(
                "[TEST] Goals has a bad bumber of strategies: %d" % len(goal.strategies)
            )
            return False
        ...
        if len(strategy.goals) != 2:
            print(
                "[TEST] Strategy A has a bad bumber of goal: %d" % len(strategy.goals)
            )
            passed = False
        ...
        if len(strategy.goals) != 2:
            print(
                "[TEST] Strategy B has a bad bumber of goal: %d" % len(strategy.goals)
            )
            passed = False
    ...
    def eval_critical_goal(self):
        ...
        if self.goals.timeout_critical_goal != 1024:
            print(
                "[TEST] Critical timeout is not correct: %s"
                % self.goals.timeout_critical_goal
            )
            return False
