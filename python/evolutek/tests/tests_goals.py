...
class TestGoals:
    ...
    def run(self):
        ...
        for goal in list_goals:
            ...
            self.tidy_actuators()
            input(
                "please replace the robot for the next goals:\n press enter if you finish"
            )
            self.robot.recalibration(init=True)
        return True
    ...
    def push_windsocks(self, ax):
        ...
        self.cs.actuators[ROBOT].left_arm_open() if ax == 3 else cs.actuators[
            ROBOT
        ].right_arm_open()
        ...
        self.cs.actuators[ROBOT].left_arm_close() if ax == 3 else cs.actuators[
            ROBOT
        ].right_arm_close()
