...
class TrajMan(Service):
    ...
    def move_manager(self):
        ...
        while True:
            ...
            sens = (
                atan2(
                    sin(self.goal_theta - self.theta), cos(self.goal_theta - self.theta)
                )
                > 0
            )
            ...
            while not self.need_to_stop.is_set() and abs(
                self.goal_theta - self.theta
            ) > (tmp_rotmax / RATIO_ROT):
            ...
            tmp_tslmax = (
                self.tmp_trslmax if self.tmp_trslmax is not None else self.trslmax
            )
            self.tmp_trslmax = None
            ...
            while not self.need_to_stop.is_set() and dist > tmp_tslmax / RATIO_TRSL:
                ...
                new_x = self.pos.x + cos(self.theta) * tmp_tslmax / RATIO_TRSL * (
                    1 if self.sens else -1
                )
                new_y = self.pos.y + sin(self.theta) * tmp_tslmax / RATIO_TRSL * (
                    1 if self.sens else -1
                )
    ...
    def recalibration(self, sens, decal=0, set=1):
        ...
        if set:
            ...
            if pi / 4 < self.theta < 3 * pi / 4:
                self.pos.y = (
                    (decal + self.robot_size_x)
                    if not sens
                    else (3000 - decal - self.robot_size_x)
                )
            elif 3 * pi / 4 < self.theta < 7 * pi / 4:
                self.pos.x = (
                    (decal + self.robot_size_x)
                    if sens
                    else (2000 - decal - self.robot_size_x)
                )
