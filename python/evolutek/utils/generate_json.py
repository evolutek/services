...
class FileGenerator:
    ...
    def add_action(
        self, name: str, service: str, idt: str, fct: str, avoid: bool = True
    ):
    ...
    def add_subaction(
        self, name: str, args: dict = None, avoid: str = None, score: int = None
    ):
        action = {"name": name}
        if args is not None:
            action["args"] = args
    ...
    def add_goto_xy_subaction(
        self, x: int, y: int, avoid: bool = True, avoid_strategy=None
    ):
...
fg.add_goal("Wait for coffee", 0).add_path(450, 225).add_goto_xy_subaction(
    250, 225, False
).add_subaction("wait", {"time": 4})

fg.add_goal("Get 3 palets", 0).add_path(250, 225).add_goto_xy_subaction(
    1860, 225, False
).add_subaction(
    "move_trsl", {"dest": 60, "acc": 100, "dec": 100, "maxspeed": 500, "sens": 1}
).add_subaction(
    "free"
).add_subaction(
    "set_x", {"x": 1880}
    ...
    "set_theta", {"theta": 0}
).add_goto_xy_subaction(
    1680, 225, False
).add_subaction(
    "get_palet"
).add_goto_xy_subaction(
    1360, 225, False
).add_goto_theta_subaction(
    "-pi/2", False
)
"""
...
# .add_path(1265, 800)\
fg.add_goal("Get blue palet and drop it", 0).add_path(
    1360, 1350
).add_goto_theta_subaction("-pi/2", False).add_subaction(
    "move_trsl", {"dest": 70, "acc": 100, "dec": 100, "maxspeed": 500, "sens": 0}
).add_subaction(
    "free"
).add_subaction(
    "set_y", {"y": 1360}
).add_subaction(
    "set_theta", {"theta": "-pi/2"}
).add_subaction(
    "move_trsl", {"dest": 70, "acc": 100, "dec": 100, "maxspeed": 500, "sens": 1}
)
fg.add_goal("Push Blue Palet", 20).add_path(1000, 1320).add_goto_xy_subaction(
    600, 1620, True
).add_goto_theta_subaction(0, False).add_goto_xy_subaction(
    150, 1625, False
).add_subaction(
    "move_trsl", {"dest": 60, "acc": 100, "dec": 100, "maxspeed": 500, "sens": 0}
).add_subaction(
    "push_ejecteur"
).add_subaction(
    "reset_ejecteur"
    ...
    "move_trsl", {"dest": 60, "acc": 500, "dec": 500, "maxspeed": 500, "sens": 1}
).add_goto_xy_subaction(
    350, 1625, True
)
...
fg.add_goal("Get Goldenium", 30).add_path(350, 2225).add_goto_theta_subaction(
    "pi", False
).add_goto_xy_subaction(260, 2225, False).add_goto_theta_subaction(
    "pi", False
).add_subaction(
    "drop_palet_rampe"
).add_goto_xy_subaction(
    ...
    "get_goldenium", avoid="Avoid.Skip", score=20
).add_goto_xy_subaction(
    350, 2225, True
)

fg.add_goal("Drop Goldenium", 32).add_path(1050, 1315).add_goto_theta_subaction(
    "0", False
).add_goto_xy_subaction(1400, 1315, False).add_subaction("drop_goldenium")

fg.add_goal("Push leftover palets", 6).add_path(1050, 1315).add_path(
    550, 600
).add_goto_theta_subaction("-pi/2").add_subaction("drop_palet")
