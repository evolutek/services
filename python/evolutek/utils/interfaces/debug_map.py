...
class Interface:
    def __init__(self, map, service):
        ...
        self.canvas.create_image(
            int(self.width / 2), int(self.height / 2), image=self.image
        )
        if nb_tims > 0:
    ...
    def print_map(self):
        ...
        for poly in self.map.color_obstacles:
            self.print_polygon(
                self.map.color_obstacles[poly].exterior.coords, ObstacleType.color
            )
        for poly in self.map.robots:
            self.print_polygon(
                self.map.robots[poly].exterior.coords, ObstacleType.robot
            )
    ...
    def print_raw_data(self, raw_data):
        ...
        for p in raw_data:
            # print(p)
            self.canvas.create_rectangle(
                p.y * unit, p.x * unit, p.y * unit + 5, p.x * unit + 5, fill="white"
            )
    ...
    def print_shapes(self, shapes):
        ...
        for i in range(len(shapes)):
            ...
            for p in shapes[i]:
                self.canvas.create_rectangle(
                    p.y * unit, p.x * unit, p.y * unit + 5, p.x * unit + 5, fill=color
                )
    ...
    def print_robots(self, robots):
        ...
        for i in range(len(robots)):
            ...
            if self.service.debug:
                self.canvas.create_rectangle(
                    p.y * unit, p.x * unit, p.y * unit + 10, p.x * unit + 10, fill=color
                )
            else:
                self.canvas.create_rectangle(
                    (p["y"] - self.service.robot_size) * unit,
                    (p["x"] - self.service.robot_size) * unit,
                    (p["y"] + self.service.robot_size) * unit,
                    (p["x"] + self.service.robot_size) * unit,
                    fill="red",
                )
    ...
    def update(self):
        ...
        self.canvas.create_image(
            self.width * unit, self.height * unit, image=self.image
        )
        if self.service.debug:
