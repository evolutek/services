...
from evolutek.lib.settings import ROBOT, SIMULATION
...
class AIInterface(Interface):
    def __init__(self):
        ...
        self.match_status_watchdog = Watchdog(
            3, self.reset_match_status
        )  # float(match_config['refresh']) * 2, self.reset_match_status)
        if SIMULATION:
    ...
    def init_simulation(self):
        ...
        for enemy, config in enemies["robots"].items():
            self.robots[enemy] = {
                "telemetry": None,
                "size": config["config"]["robot_size_y"],
                "color": "red",
            }
    ...
    def action_strategy(self):
        self.client.publish(
            ROBOT + "_strategy", strategy=self.select_strategy.get(), ai=ROBOT
        )
    ...
    def init_interface(self):
        ...
        self.color_button = Button(
            self.window, text="Change color", command=self.action_color
        )
        ...
        self.reset_button = Button(
            self.window, text="Reset Match", command=self.reset_match
        )
        self.reset_button.grid(row=8, column=0)

        self.shutdown_button = Button(
            self.window, text="shutdown", command=self.shutdown
        )
        self.shutdown_button.grid(row=10, column=0)

        self.recalibration_button = Button(
            self.window, text="Recalibration", command=self.event_recalibration
        )
        self.recalibration_button.grid(row=11, column=0)

        # Reset Button
        self.resset_pos = Button(
            self.window, text="Reset position", command=self.event_set_pos
        )
        ...
        self.canvas = Canvas(
            self.window,
            width=3000 * self.interface_ratio,
            height=2000 * self.interface_ratio,
        )
        ...
        self.canvas.create_image(
            1500 * self.interface_ratio, 1000 * self.interface_ratio, image=self.map
        )
    ...
    def update_interface(self):
        ...
        if self.cs.match.get_status() is not None:
            self.color_label.config(
                text="Color: %s" % self.cs.match.get_color(),
                fg=self.cs.match.get_status()["color"],
            )
            self.score_label.config(
                text="Score: %d" % self.cs.match.get_status()["score"]
            )
            self.match_status_label.config(
                text="Match status: %s" % self.cs.match.get_status()["status"]
            )
            self.match_time_label.config(
                text="Match time: %d" % self.cs.match.get_status()["time"]
            )
