...
class MapInterface(Interface):
    def __init__(self):
        ...
        self.client.add_subscribe_cb(
            "tim_detected_robots", self.tim_detected_robots_handler
        )
    ...
    def init_interface(self):
        ...
        self.map_button = Button(
            self.window,
            text="Display Map",
            command=self.enable_map,
            bg="green" if self.map_enabled else "red",
        )
        self.map_button.grid(row=1, column=4)

        # TIM enable Button
        self.tim_enable_button = Button(
            self.window,
            text="Display TIM",
            command=self.enable_tim,
            bg="green" if self.tim_enabled else "red",
        )
        ...
        self.tim_debug_button = Button(
            self.window,
            text="Change TIM Debug level: %s" % list(DebugMode)[self.tim_debug].name,
