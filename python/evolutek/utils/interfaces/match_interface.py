...
class MatchInterface(Interface):
    ...
    def update_interface(self):
        ...
        self.pal_ai_status_label.config(
            text="PAL status: %s" % self.get_robot_status("pal")
        )
        self.pmi_ai_status_label.config(
            text="PMI status: %s" % self.get_robot_status("pmi")
        )

        if self.cs.match.get_status() is not None:
            self.color_label.config(
                text="Color: %s" % self.cs.match.get_status()["color"],
            ...
            self.match_status_label.config(
                text="Match status: %s" % "Match not connected"
            )
