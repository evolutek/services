...
def robot_shell(ctx):
    if ctx.invoked_subcommand is None:
        ...
        subshell = make_click_shell(
            ctx, prompt=get_prompt, intro="Shell to control Robot"
        )
        subshell.cmdloop()
    else:
...
@click.option(
    ...
    "--block/--no-block",
    default=False,
    help="Makes the operation blocking (not async)",
)
@click.option(
    "-m",
    "--mirror/--no-mirror",
    default=False,
    help="Mirrors the position of the robot",
)
@click.option(
    "-a", "--avoid/--no-avoid", default=False, help="Makes the robot use avoid"
)
@click.option(
    "-p",
    "--pathfinding/--no-pathfinding",
    default=False,
    help="Makes the robot use the pathfinding",
)
...
@click.option(
    "-b", "--block/--no-block", default=False, help="if call will be blocking"
)
