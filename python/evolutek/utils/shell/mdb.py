...
def mdb_shell(ctx):
    if ctx.invoked_subcommand is None:
        ...
        subshell = make_click_shell(
            ctx, prompt=get_prompt, intro="Shell to control Mdb"
        )
        subshell.cmdloop()
    else:
...
def color(v):
    ...
    if c is not None:
        ...
        click.echo("Unknown color. Possible values are yellow, blue, y, b")
...
def mode(v):
    v = v.lower()
    if v in ["distances", "d"]:
        tm.set_mdb_config(mode=0)
    elif v in ["zones", "z"]:
        tm.set_mdb_config(mode=1)
    elif v in ["loading", "l"]:
        tm.set_mdb_config(mode=2)
    elif v in ["disabled", "db"]:
        tm.set_mdb_config(mode=3)
    else:
        click.echo(
            "Unknown mode. Possible values are distances, zones, loading, disabled, d, z, l, db"
        )
